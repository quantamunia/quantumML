...
class Calculations:
    ...
    def run_quantum_job(self):
        if self.var_circuit is not None:
            ...
            # Calculate the expected value of the Pauli-Z operator
            expected_value_z = sum((-1)**int(state[-1]) * count / total_measurements for state, count in counts.items())
            ...
            'Entropy': entropy,
            'ExpectedValueZ': expected_value_z,
...
def calculate_metrics():
    # Create an instance of Job_quantum
    job_quantum = Job_quantum()

    # Calculate the metrics
    metrics = job_quantum.calculate_metrics()
